...
def selected_stock_calculate_performance(returns, initial_value, start_date, end_date, portfolio_daily_returns):
    ...
    portfolio_daily_returns_mean = np.mean(portfolio_daily_returns)
    final_value =  (1 + portfolio_daily_returns_mean / 100)

    # Calculate CAGR
    CAGR = ((final_value / initial_value) ** (1 / t)) - 1
    ...
    dd_volatility = calculate_dd_volatility(portfolio_daily_returns)
...
if st.button("Calculate"):
    ...
    df_nifty = pd.DataFrame(nifty_data, index=["Strategy", "Benchmark"])

    CAGR1, volatility1, sharpe_ratio1, max_dd1, dd_volatility1 = selected_stock_calculate_performance(portfolio_returns, initial_value, start_date, end_date, portfolio_dailyreturns)
    selected_stocks_performance = {'CAGR (%)': [round(CAGR1, 2)], 'Volatility (%)': [round(volatility1, 2)], 'Sharpe Ratio': [round(sharpe_ratio1, 2)],'MaxDD (%)': [round(max_dd1, 2)],
                  'DDVol (%)': [round(dd_volatility1, 2)], "Start Date": start_date, "End Date": end_date}
